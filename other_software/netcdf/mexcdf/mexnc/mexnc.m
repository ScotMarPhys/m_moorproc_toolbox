...
%    "User's Guide for netCDF".  This documentation may be obtained from Unidata 
%    at <http://my.unidata.ucar.edu/content/software/netcdf/docs.html>.
%
%    R2008b and Beyond
%    -----------------
%    Starting with R2008b, MATLAB comes with native netCDF support.  In
%    order to use this native support, you must set a preference:
%
%        setpref('MEXNC','USE_TMW',true);
%
%    The general syntax for MEXNC is mexnc(funcstr,param1,param2,...). There is 
%    a one-to-one correspondence between functions in the netCDF library and
%    valid values for funcstr.  For example, MEXNC('close',ncid) corresponds to 
%    the C library call nc_close(ncid).
%
%    Syntax conventions
%    ------------------ 
...
%    return status of the corresponding netCDF API function.   A non-zero value
%    corresponds to an error.  You can use mexnc('STRERROR',status) 
...
if getpref('MEXNC','USE_TMW',false)
	use_tmw = true;
...
switch op
    case 'close'
        [varargout{:}] = handle_close ( varargin{:} );

    case 'copy_att'
        [varargout{:}] = handle_copy_att ( varargin{:} );

    case 'create'
        [varargout{:}] = handle_create_tmw ( varargin{:} );

    case '_create'
        [varargout{:}] = handle__create_tmw ( varargin{2:end} );

    case 'def_dim'
        [varargout{:}] = handle_def_dim(varargin{:});

    case 'def_var'
        [varargout{:}] = handle_def_var(varargin{:});

    case 'del_att'
        [varargout{:}] = handle_del_att(varargin{:});

    case {'enddef', 'end_def', '_enddef'}
        [varargout{:}] = handle_enddef(varargin{:});
    ...
    case 'inq'
        [varargout{:}] = handle_inq(varargin{:});

    case 'inq_ndims'
        [varargout{:}] = handle_inq_ndims(varargin{:});

    case 'inq_nvars'
        [varargout{:}] = handle_inq_nvars(varargin{:});

    case 'inq_natts'
        [varargout{:}] = handle_inq_natts(varargin{:});

    case 'inq_dim'
        [varargout{:}] = handle_inq_dim(varargin{:});

    case 'inq_dimlen'
        [varargout{:}] = handle_inq_dimlen(varargin{:});

    case 'inq_dimname'
        [varargout{:}] = handle_inq_dimname(varargin{:});

    case 'inq_attid'
        [varargout{:}] = handle_inq_attid(varargin{:});

    case 'inq_dimid'
        [varargout{:}] = handle_inq_dimid(varargin{:});

    case 'inq_libvers'
        [varargout{:}] = netcdf.inqLibVers();

    case 'inq_var'
        [varargout{:}] = handle_inq_var(varargin{:});

    case 'inq_varname'
        [varargout{:}] = handle_inq_varname(varargin{:});

    case 'inq_vartype'
        [varargout{:}] = handle_inq_vartype(varargin{:});

    case 'inq_varndims'
        [varargout{:}] = handle_inq_varndims(varargin{:});

    case 'inq_vardimid'
        [varargout{:}] = handle_inq_vardimid(varargin{:});

    case 'inq_varnatts'
        [varargout{:}] = handle_inq_varnatts(varargin{:});

    case 'inq_varid'
        [varargout{:}] = handle_inq_varid(varargin{:});

    case 'inq_att'
        [varargout{:}] = handle_inq_att(varargin{:});

    case 'inq_atttype'
        [varargout{:}] = handle_inq_atttype(varargin{:});

    case 'inq_attlen'
        [varargout{:}] = handle_inq_attlen(varargin{:});

    case 'inq_attname'
        [varargout{:}] = handle_inq_attname(varargin{:});

    case 'inq_unlimdim'
        [varargout{:}] = handle_inq_unlimdim(varargin{:});

    case 'open'
        [varargout{:}] = handle_open(varargin{:});

    case '_open'
        [varargout{:}] = handle__open(varargin{:});

    case { 'put_att_double', 'put_att_float', 'put_att_int', 'put_att_short', ...
	       'put_att_schar', 'put_att_uchar', 'put_att_text' }
        [varargout{:}] = handle_put_att(varargin{:});

    case { 'put_var_double', 'put_var_float', 'put_var_int', ...
	       'put_var_short', 'put_var_schar', 'put_var_uchar', ...
		   'put_var_text' }
        [varargout{:}] = handle_put_var(varargin{:});

    case { 'put_var1_double', 'put_var1_float', 'put_var1_int', ...
	       'put_var1_short', 'put_var1_schar', 'put_var1_uchar', ...
		   'put_var1_text' }
        [varargout{:}] = handle_put_var1(varargin{:});

    case { 'put_vara_double', 'put_vara_float', 'put_vara_int', ...
	       'put_vara_short', 'put_vara_schar', 'put_vara_uchar', ...
		   'put_vara_text' }
        [varargout{:}] = handle_put_vara(varargin{:});

    case { 'put_vars_double', 'put_vars_float', 'put_vars_int', ...
	       'put_vars_short', 'put_vars_schar', 'put_vars_uchar', ...
		   'put_vars_text' }
        [varargout{:}] = handle_put_vars(varargin{:});
    ...
    case 'redef'
        [varargout{:}] = handle_redef(varargin{:});

    case 'rename_att'
        [varargout{:}] = handle_rename_att(varargin{:});

    case 'rename_dim'
        [varargout{:}] = handle_rename_dim(varargin{:});

    case 'rename_var'
        [varargout{:}] = handle_rename_var(varargin{:});

    case 'set_fill'
        [varargout{:}] = handle_set_fill(varargin{:});

    case 'strerror'
        [varargout{:}] = handle_strerror(varargin{:});

    case 'sync'
        [varargout{:}] = handle_sync(varargin{:});

	% NETCDF-2 functions
    case 'attcopy'
        [varargout{:}] = handle_attcopy ( varargin{:} );

    case 'attdel'
        [varargout{:}] = handle_attdel ( varargin{:} );

    case 'attget'
        [varargout{:}] = handle_attget ( varargin{:} );

    case 'attinq'
        [varargout{:}] = handle_attinq ( varargin{:} );

    case 'attname'
        [varargout{:}] = handle_attname ( varargin{:} );

    case 'attput'
        [varargout{:}] = handle_attput ( varargin{:} );

    case 'attrename'
        [varargout{:}] = handle_attrename ( varargin{:} );

    case 'dimdef'
        [varargout{:}] = handle_dimdef ( varargin{:} );

    case 'dimid'
        [varargout{:}] = handle_dimid ( varargin{:} );

    case 'diminq'
        [varargout{:}] = handle_diminq ( varargin{:} );

    case 'dimrename'
        [varargout{:}] = handle_dimrename ( varargin{:} );

    case 'endef'
        [varargout{:}] = handle_endef ( varargin{:} );

    case 'inquire'
        [varargout{:}] = handle_inquire ( varargin{:} );

    case 'typelen'
        [varargout{:}] = handle_typelen ( varargin{:} );

    case 'vardef'
        [varargout{:}] = handle_vardef ( varargin{:} );

    case 'varid'
        [varargout{:}] = handle_varid ( varargin{:} );

    case 'varinq'
        [varargout{:}] = handle_varinq ( varargin{:} );

    case 'varget1'
        [varargout{:}] = handle_varget1 ( varargin{:} );

    case 'varput1'
        [varargout{:}] = handle_varput1 ( varargin{:} );

    case 'varget'
        [varargout{:}] = handle_varget ( varargin{:} );

    case 'varput'
        [varargout{:}] = handle_varput ( varargin{:} );

    case 'vargetg'
        [varargout{:}] = handle_vargetg ( varargin{:} );

    case 'varputg'
        [varargout{:}] = handle_varputg ( varargin{:} );

    case 'varrename'
        [varargout{:}] = handle_varrename ( varargin{:} );

    case 'setopts'
        [varargout{:}] = handle_setopts(varargin{:});
...
%------------------------------------------------------------------------------------------
function varargout = handle_create_tmw ( varargin )
%      [ncid,status] = mexnc ('CREATE',filename,access_mode );
...
%------------------------------------------------------------------------------------------
function varargout = handle__create_tmw ( varargin )
% [chunksz_out,ncid,status] = mexnc ('_CREATE',filename,mode,initialsize,chunksz_in);
...
filename = varargin{1};
mode = varargin{2};
initialsize = varargin{3};
if nargin == 4
    chunksize_in = varargin{4};
else
    chunksize_in = 0;
end

try
    [czout,ncid,status] = netcdf.create(filename,mode,initialsize,chunksize_in);
    status = 0;
catch myException
    czout = -1;
    ncid = -1;
...
funcstr = getpref('MEXNC','FUNCSTR','vanilla_mexnc');

varargout = cell(1,nargout);

if nargout > 0
    [varargout{:}] = feval(funcstr, varargin{:});
else
    feval(funcstr, varargin{:});
end
