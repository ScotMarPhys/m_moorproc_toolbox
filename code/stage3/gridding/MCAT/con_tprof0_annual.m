...
%   PCM-1 Array
% 
%       Notes from bim (March 2012).
%	  
%   must have a minimum of 6 microcats
%
...
%
% function [t_con,s_con] = con_tprof0(tg,sg,pg,PG,time,int_step,TSclim)
%    
%   input:
%      tg       = mooring temperature time series (stored as rows in tg)
%                 rows are the microcats at different depths (e.g. 12 x 5515)
...
%                 correspnding to tg
%                 all values passed to the function are zero
...
%
%   output:
%      t_con / s_con   = output temperature interpolated onto the pressure grid (PG)
%
...
function [t_con,s_con] = con_tprof0_v3(tg,sg,pg,PG,time,int_step,TSclim,TS_CLIMATOLOGY,TS_CLIMATOLOGY_NAME)

% why bring the gridded pressure in and then clear it and reset ?
clear PG
PG = [0: 20: 4820]';
p_gridsize = 20;
...
% find the maximum depths used
	depths=unique(pmax);depths=depths(find(~isnan(depths)));
...
% --- load lookup tables for dt/dp climatology --------
        disp(['loading ' TSclim ' dtdp climatology'])
eval(['load ',TSclim])
...
%	TS_CLIMATOLOGY will be slope, mareast, marwest or wall.
	...
	['cSg =dsdp_' TS_CLIMATOLOGY '_hbase' ]
	['cTg =dtdp_' TS_CLIMATOLOGY '_hbase' ]
	['cT =tgrid_hbase' ]
       eval( ['cSg =dsdp_' TS_CLIMATOLOGY '_hbase;' ] );
       eval( ['cTg =dtdp_' TS_CLIMATOLOGY '_hbase;' ] );
       eval( ['cT =tgrid_hbase;' ] );
...
%   create two columns with length of PG
...
% temperature between gridpoints 
...
%      t_bound0(T(1),S(1),P(1),time(ti),int_step,pmin,cTg,cSg,cT,tTg,tT,tt);% temperature above
      ...
      innerP = [upP;innerP];      
      innerS = [upS;innerS];  
    end % end of if(P(1) > pmin(ti)) loop   
...
%
% function [innerT,innerS,innerP] = t_int0(t,s,p,time,step,cTg,cSg,cT,tTg,tT,tt,preverse)
%
...
%         cTg -- constant gradient [C/dbar] for : 2.3 < T < 18 (dT/dP) from climatology
...
% store two successive pressure, temperature and salinity measurememnts
...
% (i.e., temperature increases with depth),
