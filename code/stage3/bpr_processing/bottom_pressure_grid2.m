...
for sensor = 1 :length(sn)
  ...
  if ~exist(infile)
    ...
    error('*.use file does not exist - breaking bottom_pressure_grid2.m!')
    break
    
  end   

  outfile = [basedir moor '/' ext(2:end) '/' moor '_' sprintf('%3.3d',sensor) ext];


  % initialize the log file
  
  logfile = [infile(1:end-4) '_stage3_log'];
