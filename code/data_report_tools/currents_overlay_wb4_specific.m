function currents_overlay_wb4_specific(moor,varargin)
%
...
%
% function currents_overlay('moor','layout','plot_interval','procpath')
%
% required inputs:-
...
if nargin <1
    help currents_overlay_wb4-specific
    return
end
...
a=strmatch('plot_interval',varargin_string,'exact'); % check for plot_interval first
if a>0
    plot_interval=varargin{a+1};
    if a+1<length(varargin) % remove plot_interval from varargin and varargin_string if not last input
        varargin={varargin{1:a-1},varargin{a+2:end}};
        varargin_string={varargin_string{1:a-1},varargin_string{a+2:end}};
    end
else
    plot_interval=0;
end
...
while check~=1
    ...
    elseif (plot_interval(i,1)==plot_interval(i-1,1)) & (plot_interval(i,2)==plot_interval(i-1,2))
        check = 1;
...
iii=[iiiS4;iiiRCM11;iiiARG;iiiNOR];
...
%     end
    ...
    line_style = '-'; %wb4 specific

    figure(direction_plot); hold on
    a=sampling_rate/num_to_plot; %value to decimate data by to ease plot visualisation doesn't actually average it, just decimates
...
set(gca,'XTick',jdxticks-jd1);
s = regexprep(moor,'_','\\_');
title(['Current speed of low pass filtered currents at mooring ' s '.'])
legend(legend_text);
...
set(gca,'xticklabel',{'14/6/07', '15/6/07', '16/6/07', '17/6/07', '18/6/07', '19/6/07', '20/6/07'})
hold on
plot([(202/288)*6,(202/288)*6],[0 50],'k-') % implied implosion time of steel sphere
set(gca,'ylim',[0 50])
