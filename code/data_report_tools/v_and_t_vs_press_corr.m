function v_and_t_vs_press_corr(moor,varargin)
%
% Function for calculating and plotting the correlation betwen northward 
% velocity and pressure, and for temperature and pressure.
% - Suggested by Harry for checking if any
% correlation for heat transport estimates - this time to check if
% knockdown is prime contributor to changes in the temperature changes
%
% Also ouputs to screen the variance and covariances of the different
% comparisons.
%
% NB: currently using temp and not potemp.
%
% function v_and_t_vs_press_corr('moor','layout','procpath','"path"','outpath','"path"')
%
% required inputs:-
...
%                inspection
%   outpath: path to save figures and stats file to (default = current
%            directory './'
%
% functions called:-
...
% 
% Routine written by Darren Rayner Feb 2010 - adapted from vel_vs_temp_corr.m.


if nargin <1
    help v_and_t_vs_press_corr
    return
end
...
if a>0
    layout=str2double(varargin_string{a+1});
else
    layout='portrait';
end
...
a=strmatch('outpath',varargin_string,'exact');
if a>0
...
correlation_plot1=figure('Position',pos1);
correlation_plot2=figure('Position',pos1);
 
set(correlation_plot1,'PaperUnits','centimeters');
set(correlation_plot1, 'PaperType', 'A4');
set(correlation_plot1, 'PaperOrientation',layout);
set(correlation_plot2,'PaperUnits','centimeters');
set(correlation_plot2, 'PaperType', 'A4');
set(correlation_plot2, 'PaperOrientation',layout);
papersize = get(correlation_plot1,'PaperSize');
width=17; height=26; left = (papersize(1)- width)/2; bottom = (papersize(2)- height)/2;
figuresize = [left, bottom, width, height];
set(correlation_plot1, 'PaperPosition', figuresize);
set(correlation_plot2, 'PaperPosition', figuresize);
%%
plot_string={};
...
%%
% Section to find if Microcats are within certain depth of the current
% meter and if so interpolate onto common timebase
...
% create regular time space and interpolate those instruments that are at a
% matching depth onto the new timespace
jd_new=jd_start:(1/48):(jd_end);
b=[matching_insts zeros(a(1)*2,1)];
b(1:end,4)=1:1:a(1)*2;
for i=1:a(1)*2
   if matching_insts(i,2)==310 % RCM11
       eval(['RCM11_' num2str(matching_insts(i,1)) '.u2=interp1(RCM11_' num2str(matching_insts(i,1)) '.jd, RCM11_' num2str(matching_insts(i,1)) '.u, jd_new,''linear'');'])
       eval(['RCM11_' num2str(matching_insts(i,1)) '.v2=interp1(RCM11_' num2str(matching_insts(i,1)) '.jd, RCM11_' num2str(matching_insts(i,1)) '.v, jd_new,''linear'');'])
       eval(['RCM11_' num2str(matching_insts(i,1)) '.t2=interp1(RCM11_' num2str(matching_insts(i,1)) '.jd, RCM11_' num2str(matching_insts(i,1)) '.t, jd_new,''linear'');'])
       eval(['RCM11_' num2str(matching_insts(i,1)) '.p2=interp1(RCM11_' num2str(matching_insts(i,1)) '.jd, RCM11_' num2str(matching_insts(i,1)) '.p, jd_new,''linear'');'])
   elseif matching_insts(i,2)==302 % S4
       eval(['S4_' num2str(matching_insts(i,1)) '.u2=interp1(S4_' num2str(matching_insts(i,1)) '.jd, S4_' num2str(matching_insts(i,1)) '.u, jd_new,''linear'');'])
       eval(['S4_' num2str(matching_insts(i,1)) '.v2=interp1(S4_' num2str(matching_insts(i,1)) '.jd, S4_' num2str(matching_insts(i,1)) '.v, jd_new,''linear'');'])
       eval(['S4_' num2str(matching_insts(i,1)) '.p2=interp1(S4_' num2str(matching_insts(i,1)) '.jd, S4_' num2str(matching_insts(i,1)) '.p, jd_new,''linear'');'])
       eval(['S4_' num2str(matching_insts(i,1)) '.t2=interp1(S4_' num2str(matching_insts(i,1)) '.jd, S4_' num2str(matching_insts(i,1)) '.t, jd_new,''linear'');'])
   elseif (matching_insts(i,2)==366 || matching_insts(i,2)==366337) % Argonaut
       eval(['ARG_' num2str(matching_insts(i,1)) '.u2=interp1(ARG_' num2str(matching_insts(i,1)) '.jd, ARG_' num2str(matching_insts(i,1)) '.u, jd_new,''linear'');'])
       eval(['ARG_' num2str(matching_insts(i,1)) '.v2=interp1(ARG_' num2str(matching_insts(i,1)) '.jd, ARG_' num2str(matching_insts(i,1)) '.v, jd_new,''linear'');'])
       eval(['ARG_' num2str(matching_insts(i,1)) '.t2=interp1(ARG_' num2str(matching_insts(i,1)) '.jd, ARG_' num2str(matching_insts(i,1)) '.t, jd_new,''linear'');'])
       eval(['ARG_' num2str(matching_insts(i,1)) '.p2=interp1(ARG_' num2str(matching_insts(i,1)) '.jd, ARG_' num2str(matching_insts(i,1)) '.p, jd_new,''linear'');'])
   elseif matching_insts(i,2)==368 % Nortek
       eval(['NOR_' num2str(matching_insts(i,1)) '.u2=interp1(NOR_' num2str(matching_insts(i,1)) '.jd, NOR_' num2str(matching_insts(i,1)) '.u, jd_new,''linear'');'])
       eval(['NOR_' num2str(matching_insts(i,1)) '.v2=interp1(NOR_' num2str(matching_insts(i,1)) '.jd, NOR_' num2str(matching_insts(i,1)) '.v, jd_new,''linear'');'])
       eval(['NOR_' num2str(matching_insts(i,1)) '.t2=interp1(NOR_' num2str(matching_insts(i,1)) '.jd, NOR_' num2str(matching_insts(i,1)) '.t, jd_new,''linear'');'])
       eval(['NOR_' num2str(matching_insts(i,1)) '.p2=interp1(NOR_' num2str(matching_insts(i,1)) '.jd, NOR_' num2str(matching_insts(i,1)) '.p, jd_new,''linear'');'])
   elseif matching_insts(i,2)==337 % MicroCAT - interpolate temp and pressure, not velocity for MicroCAT
       eval(['MC_' num2str(matching_insts(i,1)) '.t2=interp1(MC_' num2str(matching_insts(i,1)) '.jd, MC_' num2str(matching_insts(i,1)) '.t, jd_new,''linear'');'])
       eval(['MC_' num2str(matching_insts(i,1)) '.p2=interp1(MC_' num2str(matching_insts(i,1)) '.jd, MC_' num2str(matching_insts(i,1)) '.p, jd_new,''linear'');'])
   end
end
...
cm_sn=sn(iii);
if filt_or_unfilt==0 % filtered
    stats_file=[outpath moor '_var_and_covar_2-day-filt.asc'];
else
    stats_file=[outpath moor '_var_and_covar_unfilt.asc'];
end
for i=1:length(iii)
    %% figure 1
    figure(correlation_plot1);
    h(i)=subplot(m,n,i);
    hold on
    
    mean_p=eval(['nanmean(' char(eval_string2(i)) '.p)']);
    mean_v=eval(['nanmean(' char(eval_string2(i)) '.v)']);
    mean_t=eval(['nanmean(' char(eval_string2(i)) '.t)']);
       
    
    eval(['plot(' char(eval_string2(i)) '.p-mean_p,' char(eval_string2(i)) '.v-mean_v,''.'');'])
    % now overlay Microcats if close by.
    b=find(cm_sn(i)==matching_insts(:,1)); 
    if b>0 % means has a microcat nearby
        c=find(matching_insts(:,3)==matching_insts(b,3)); % checks in case matching MC is above or below cm in matching_insts array
        c=setdiff(c,b); % removes original index of cm so only have MC indexed
        mean_p_MC=eval(['nanmean(MC_' num2str(matching_insts(c,1)) '.p2)']); 
        mean_v2=eval(['nanmean(' char(eval_string2(i)) '.v2)']);
        eval(['plot(MC_' num2str(matching_insts(c,1)) '.p2-mean_p_MC,' char(eval_string2(i)) '.v2-mean_v2,''r.'');'])
    end
    if b>0 % i.e. has nearby Microcat
        title_string=[char(eval_string2(i)) '.  MC_' num2str(matching_insts(c,1))];
    else
        title_string=char(eval_string2(i));
    end
    title_string=regexprep(title_string,'_','\\_');
    title(['Current meter at ' num2str(depths(iii(i),2)) 'm. ' title_string]);
    xlabel('pressure anomaly (dbar)')
    ylabel('v velocity anomaly component (cm/s)')
    
    %% figure2
    figure(correlation_plot2);
    h2(i)=subplot(m,n,i);
    hold on
    eval(['plot(' char(eval_string2(i)) '.p-mean_p,' char(eval_string2(i)) '.t-mean_t,''.'');'])
    ...
    if b>0 % means has a microcat nearby
        c=find(matching_insts(:,3)==matching_insts(b,3)); % checks in case matching MC is above or below cm in matching_insts array
        c=setdiff(c,b); % removes original index of cm so only have MC indexed
        mean_t_MC=eval(['nanmean(MC_' num2str(matching_insts(c,1)) '.t2)']); 
        mean_p_MC=eval(['nanmean(MC_' num2str(matching_insts(c,1)) '.p2)']); 
        eval(['plot(MC_' num2str(matching_insts(c,1)) '.p2-mean_p_MC,MC_' num2str(matching_insts(c,1)) '.t2-mean_t_MC,''r.'');'])
    end
    if b>0 % i.e. has nearby Microcat
        title_string=[char(eval_string2(i)) '.  MC_' num2str(matching_insts(c,1))];
    else
    ...
    %%
    % Calculation and output to file of variance and covariance
    
    eval(['variance.' char(eval_string2(i)) '.u_prime=nanvar(' char(eval_string2(i)) '.u-nanmean(' char(eval_string2(i)) '.u));'])
    eval(['variance.' char(eval_string2(i)) '.v_prime=nanvar(' char(eval_string2(i)) '.v-nanmean(' char(eval_string2(i)) '.v));'])
    eval(['variance.' char(eval_string2(i)) '.t_prime=nanvar(' char(eval_string2(i)) '.t-nanmean(' char(eval_string2(i)) '.t));'])
    eval(['variance.' char(eval_string2(i)) '.p_prime=nanvar(' char(eval_string2(i)) '.p-nanmean(' char(eval_string2(i)) '.p));'])
    
    eval(['covariance.' char(eval_string2(i)) '.uv_prime=nancov((' char(eval_string2(i)) '.u-nanmean(' char(eval_string2(i)) '.u)),(' char(eval_string2(i)) '.v-nanmean(' char(eval_string2(i)) '.v)));'])
    eval(['covariance.' char(eval_string2(i)) '.ut_prime=nancov((' char(eval_string2(i)) '.u-nanmean(' char(eval_string2(i)) '.u)),(' char(eval_string2(i)) '.t-nanmean(' char(eval_string2(i)) '.t)));'])
    eval(['covariance.' char(eval_string2(i)) '.up_prime=nancov((' char(eval_string2(i)) '.u-nanmean(' char(eval_string2(i)) '.u)),(' char(eval_string2(i)) '.p-nanmean(' char(eval_string2(i)) '.p)));'])
    eval(['covariance.' char(eval_string2(i)) '.vt_prime=nancov((' char(eval_string2(i)) '.v-nanmean(' char(eval_string2(i)) '.v)),(' char(eval_string2(i)) '.t-nanmean(' char(eval_string2(i)) '.t)));'])
    eval(['covariance.' char(eval_string2(i)) '.vp_prime=nancov((' char(eval_string2(i)) '.v-nanmean(' char(eval_string2(i)) '.v)),(' char(eval_string2(i)) '.p-nanmean(' char(eval_string2(i)) '.p)));'])
    eval(['covariance.' char(eval_string2(i)) '.tp_prime=nancov((' char(eval_string2(i)) '.t-nanmean(' char(eval_string2(i)) '.t)),(' char(eval_string2(i)) '.p-nanmean(' char(eval_string2(i)) '.p)));'])
    
    if i==1
        fid=fopen(stats_file,'w');
        fprintf(fid,'%s',stats_file);
    else
        fid=fopen(stats_file,'a');
    end
    
    fprintf(fid,'\n\n%s%s','Instrument: ',char(eval_string2(i)));
    eval(['fprintf(fid,''\n%s%f'',''Variance of u anomaly: '', variance.' char(eval_string2(i)) '.u_prime);']) 
    eval(['fprintf(fid,''\n%s%f'',''Variance of v anomaly: '', variance.' char(eval_string2(i)) '.v_prime);']) 
    eval(['fprintf(fid,''\n%s%f'',''Variance of t anomaly: '', variance.' char(eval_string2(i)) '.t_prime);']) 
    eval(['fprintf(fid,''\n%s%f'',''Variance of p anomaly: '', variance.' char(eval_string2(i)) '.p_prime);']) 

    eval(['fprintf(fid,''\n%s%f'',''Co-Variance of u and v anomalies: '', covariance.' char(eval_string2(i)) '.uv_prime(1,2));'])    
    eval(['fprintf(fid,''\n%s%f'',''Co-Variance of u and t anomalies: '', covariance.' char(eval_string2(i)) '.ut_prime(1,2));'])
    eval(['fprintf(fid,''\n%s%f'',''Co-Variance of u and p anomalies: '', covariance.' char(eval_string2(i)) '.up_prime(1,2));'])
    eval(['fprintf(fid,''\n%s%f'',''Co-Variance of v and t anomalies: '', covariance.' char(eval_string2(i)) '.vt_prime(1,2));'])
    eval(['fprintf(fid,''\n%s%f'',''Co-Variance of v and p anomalies: '', covariance.' char(eval_string2(i)) '.vp_prime(1,2));'])
    eval(['fprintf(fid,''\n%s%f'',''Co-Variance of t and p anomalies: '', covariance.' char(eval_string2(i)) '.tp_prime(1,2));'])
    
    
    if b>0 % i.e. has nearby Microcat
        fprintf(fid,'\n%s%s','Instrument: MC_',num2str(matching_insts(c,1)));
        eval(['variance.MC_' num2str(matching_insts(c,1)) '.t2_prime=nanvar(MC_' num2str(matching_insts(c,1))  '.t2-nanmean(MC_' num2str(matching_insts(c,1)) '.t2));']) 
        eval(['variance.MC_' num2str(matching_insts(c,1)) '.p2_prime=nanvar(MC_' num2str(matching_insts(c,1))  '.p2-nanmean(MC_' num2str(matching_insts(c,1)) '.p2));'])
        eval(['fprintf(fid,''\n%s%f'',''Variance of t anomaly: '', variance.MC_' num2str(matching_insts(c,1)) '.t2_prime);'])
        eval(['fprintf(fid,''\n%s%f'',''Variance of p anomaly: '', variance.MC_' num2str(matching_insts(c,1)) '.p2_prime);'])
        
        eval(['covariance.MC_' num2str(matching_insts(c,1)) 'v' char(eval_string2(i)) '.ut_prime=nancov((MC_' num2str(matching_insts(c,1)) '.t2-nanmean(MC_' num2str(matching_insts(c,1)) '.t2)),(' char(eval_string2(i)) '.u2-nanmean(' char(eval_string2(i)) '.u2)));'])
        eval(['covariance.MC_' num2str(matching_insts(c,1)) 'v' char(eval_string2(i)) '.up_prime=nancov((MC_' num2str(matching_insts(c,1)) '.p2-nanmean(MC_' num2str(matching_insts(c,1)) '.p2)),(' char(eval_string2(i)) '.u2-nanmean(' char(eval_string2(i)) '.u2)));'])
        eval(['covariance.MC_' num2str(matching_insts(c,1)) 'v' char(eval_string2(i)) '.vt_prime=nancov((MC_' num2str(matching_insts(c,1)) '.t2-nanmean(MC_' num2str(matching_insts(c,1)) '.t2)),(' char(eval_string2(i)) '.v2-nanmean(' char(eval_string2(i)) '.v2)));'])
        eval(['covariance.MC_' num2str(matching_insts(c,1)) 'v' char(eval_string2(i)) '.vp_prime=nancov((MC_' num2str(matching_insts(c,1)) '.p2-nanmean(MC_' num2str(matching_insts(c,1)) '.p2)),(' char(eval_string2(i)) '.v2-nanmean(' char(eval_string2(i)) '.v2)));'])
        eval(['covariance.MC_' num2str(matching_insts(c,1)) 'v' char(eval_string2(i)) '.pp_prime=nancov((MC_' num2str(matching_insts(c,1)) '.p2-nanmean(MC_' num2str(matching_insts(c,1)) '.p2)),(' char(eval_string2(i)) '.p2-nanmean(' char(eval_string2(i)) '.p2)));'])
        eval(['covariance.MC_' num2str(matching_insts(c,1)) 'v' char(eval_string2(i)) '.tt_prime=nancov((MC_' num2str(matching_insts(c,1)) '.t2-nanmean(MC_' num2str(matching_insts(c,1)) '.t2)),(' char(eval_string2(i)) '.t2-nanmean(' char(eval_string2(i)) '.t2)));'])
        eval(['fprintf(fid,''\n%s%f'',''Co-variance of u anomaly (from current meter) and t anomaly (from MicroCAT): '', covariance.MC_' num2str(matching_insts(c,1)) 'v' char(eval_string2(i)) '.ut_prime(1,2));'])
        eval(['fprintf(fid,''\n%s%f'',''Co-variance of u anomaly (from current meter) and p anomaly (from MicroCAT): '', covariance.MC_' num2str(matching_insts(c,1)) 'v' char(eval_string2(i)) '.up_prime(1,2));'])
        eval(['fprintf(fid,''\n%s%f'',''Co-variance of v anomaly (from current meter) and t anomaly (from MicroCAT): '', covariance.MC_' num2str(matching_insts(c,1)) 'v' char(eval_string2(i)) '.vt_prime(1,2));'])
        eval(['fprintf(fid,''\n%s%f'',''Co-variance of v anomaly (from current meter) and p anomaly (from MicroCAT): '', covariance.MC_' num2str(matching_insts(c,1)) 'v' char(eval_string2(i)) '.vp_prime(1,2));'])
        eval(['fprintf(fid,''\n%s%f'',''Co-variance of t anomaly (from current meter) and t anomaly (from MicroCAT): '', covariance.MC_' num2str(matching_insts(c,1)) 'v' char(eval_string2(i)) '.tt_prime(1,2));'])
        eval(['fprintf(fid,''\n%s%f'',''Co-variance of p anomaly (from current meter) and p anomaly (from MicroCAT): '', covariance.MC_' num2str(matching_insts(c,1)) 'v' char(eval_string2(i)) '.pp_prime(1,2));'])
...
if filt_or_unfilt==0 % i.e. filtered data
    suptitle_string=['2-day lowpass filtered V'' vs P'' for mooring ' s '. CM=blue, MC=red.'];
else
    suptitle_string=['Unfiltered V'' vs P'' for mooring ' s '. CM=blue, MC=red.'];
end
suptitle(suptitle_string);

figure(correlation_plot2)
if filt_or_unfilt==0 % i.e. filtered data
    suptitle_string=['2-day lowpass filtered T'' vs P'' for mooring ' s '. CM=blue, MC=red.'];
else
    suptitle_string=['Unfiltered T'' vs P'' for mooring ' s '. CM=blue, MC=red.'];
end
suptitle(suptitle_string);

%% save figures and stats summary
if filt_or_unfilt==0 % i.e. filtered
    filename1=[outpath moor '_v_vs_p_2-day-filt.eps'];
    filename2=[outpath moor '_t_vs_p_2-day-filt.eps'];
else
    filename1=[outpath moor '_v_vs_p_unfilt.eps'];
    filename2=[outpath moor '_t_vs_p_unfilt.eps'];
end

figure(correlation_plot1)
print('-depsc2', filename1);
figure(correlation_plot2)
