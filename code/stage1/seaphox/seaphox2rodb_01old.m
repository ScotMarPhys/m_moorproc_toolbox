...
%     Snernst=(R.*(MC_T).*log(10))./F;
    ...
    for k = [1 3 4 6 5]
        ...
        hold off;
        plot(jd(valI)-jd(1),data(valI,4+k));
        set(gca,'Ygrid','on')
        set(gca,'Xgrid','on')
        
        mdata=median(data(valI,4+k));
        if k == 1;
            ylabel('pH')
    ...
    %     sdt = std(dt(val(100:end-100),1));
        set(gca,'ylim',prctile(data(valI,4+k),[1 99])+[-1 1]*std(data(valI,4+k))/10)
%        set(gca,'xlim',[jd(1) jd(dl) ]-jd(1))
        set(gca,'xlim',[jd(valI(1)) jd(valI(end)) ]-jd(1))
        yl = get(gca,'ylim');
        xl = get(gca,'xlim');
