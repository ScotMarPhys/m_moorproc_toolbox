...
if iiMC>0
   ...
   write_MCTD_to_NetCDF(netcdffilepath, moor, lat, lon, mcatinfo, instrdepth, mcatdata.time(1,:),  mcatdata.p, mcatdata.s, mcatdata.t, mcatdata.c)

  save([netcdffilepath '.mat'],'moor', 'lat', 'lon', 'mcatinfo','mcatdata')
...
% Load data
   ...
   save([netcdffilepath '.mat'],'moor', 'lat', 'lon', 'nortekinfo','nortekdata')
...
if iiADCP>0
    ...
    for j=1:num_bins % loop for total number of bins
       ...
       pres = sw_pres(ADCPdata(i).z,repmat(lat,size(ADCPdata(i).z)));
       ...
       save([netcdffilepath '.mat'],'moor', 'lat', 'lon', 'adcpinfo','ADCPdata')
