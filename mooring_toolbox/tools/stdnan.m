function y = stdmiss(x)
%STDMISS Column standard deviation with missing data.
%  STDMISS(X) returns the standard deviation of each column of X as a row
%  vector where missing data values are encoded as NaNs. For vectors,
%  STDMISS(X) returns the standard deviation of the elements in X.
