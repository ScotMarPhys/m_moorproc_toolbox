% function rcm2rodb_05(moor, 'procpath', procpath, 'inpath', inpath, ...
%                      'outpath', outpath,'ctd')
%
...
%   'inpath'   = path to raw rcm files
%                no default currently set so will crash if don't provide it
%                to function input
...
% Functions used:
% rodbload.m
% rcm2rodb_z02.m   - NB: This only works with Matlab v14 or later.
...
% 23/3/10 - fixed procpath and inpaths back to what they should be. DR.
%         - fixed bug with calculating pressure for serial numbers > 500.
%           script was accidentally calling rcm2rodb_04 instead of 05.
% 
% 12/9/11 - edited paths and added cruise variable to make script run 
%           from anywhere. GDM jc064
% 04/1/13 - corrected bug where first figure was overwritten by second so
%           not saved. Now saved with *_all.eps end to filename. Also
%           changed to epsc2 format with tiff previews. DR.

function rcm2rodb_05(moor,varargin)
...
if a>0
    ...
    inpath=['/noc/users/pstar/rpdmoc/rapid/data/moor/raw/' cruise '/rcm/'];
end
disp(['inpath = ' inpath])
...
xlabel('Time [Days]')
if vec<500
    mt = median(dt(:,13)*100); % units transform to db from MPa
else
    mt = median(dt(:,13)/10); % units transform to db from kPa
end
...
if vec<500
    sal = sw_salt(c_ratio,dt(:,11),dt(:,13)*100); % P in MPa
else
    sal = sw_salt(c_ratio,dt(:,11),dt(:,13)/10); % P in kPa
end
% end SCU/JYM
...
orient tall
eval(['print -depsc2 -tiff ',outfile,'_all.eps'])
...
brush(fig_handle);
pause
% need to export data from brushing to variable "data" for output from
% function
end
