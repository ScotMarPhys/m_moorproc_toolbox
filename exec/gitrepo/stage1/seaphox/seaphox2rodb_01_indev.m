...
function seaphox2rodb_01_indev(moor, varargin)
...
%             header=textscan(fid,'%s',3,'delimiter',',');       
        for k=1:8
            header{k}=textscan(fid,'%s %s %s',1,'delimiter',',');
%             eval(['header' num2str(k) '=textscan(fid,''%s %s %s'',1,''delimiter'','','');'])
        end
         all_data=textscan(fid,'%s %f %f %s %f %s %f %f %f %f %s %f %s %f %f %f %f %f %f %f %f %f %f %s %f','delimiter',',');
        date=all_data{2};
...
%     Snernst=(R.*(MC_T).*log(10))./F;
    if strcmpi(header{5}{2},'CAL_PHEXT_OFFSET_COEFF')
        k0e=str2double(header{5}{3});
    else
    ...
    if strcmpi(header{6}{2},'CAL_PHEXT_SLOPE_COEFF')
        k2e=str2double(header{6}{3});
    else
...
% STANDARD DATA PLOT
  ...
  % LOW PASS FILTER
    ...
    for k = [1 3 4 6 5]
        ...
        y= auto_filt(ydata(id), sampling_rate, 1/2,'low',4);
        mdata=nanmedian(y);
        ...
        if k==1
            set(gca,'ylim',[mdata-nanstd(y) mdata+nanstd(y)])
        else
            set(gca,'ylim',[mdata-(3*(nanstd(y))) mdata+(3*(nanstd(y)))])
        end
            title(['SeapHOx:  ',num2str(sn(vec)),'   Depth:  ',num2str(z(vec)),' m']) 
      ...
      orient tall
      print(gcf, [outpath moor '_lowpass'  num2str(serial_nums(i))],'-dpng', '-r300');
