% function microcat2rodb_4('infile','outfile','infofile',fidlog,[graphics],[toffset])
%
...
if length(retx)<700 %in case of short record from test (added by Loic H on DY078)
    [XXX,data_length] = max(hist(diff(retx(300:end)),1:300));% length of data columns 
else
...
if cnv
    ...
    for k = 1:length(toti)-1
        ...
        if ~isempty( strfind(varstr(10), 't') ) & ~isempty( strfind(varstr, '90C') )
        ...
        elseif ~isempty(strfind(varstr(10), 'c')) & ~isempty(strfind(varstr, '0S/m'))
            ...
            condi = k;
            continue;
        elseif ~isempty(strfind(varstr, 'prM')) |  ~isempty(strfind(varstr, 'prdM'))
