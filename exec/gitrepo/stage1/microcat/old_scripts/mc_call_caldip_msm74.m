...
% AR30 SCU: Set up to re-run caldip from MSM74
% -----------------------------------------------------------------
...
if doctd == 1;
    ...
    temp=tepm;
    cond=cond*100; % units
%     new_date=datestr(timeofcast,'yyyy mm dd HH MM SS');
    [yyyy, mm, dd, HH, MM, SS] = datevec(timeofcast);
    new_time=151+dd+(HH/24)+(MM/1440)+(SS/86400);
end
...
for i = 1:length(vec)
    ...
    microcat2rodb_4_msm74(infile,outfile,infofile,fidlog,'w',dateoffsetmc)
    
    % --- load rodb data ---
    if id2(i)~=335 % checks if not ODO microcat
        [yy,mm,dd,hh,c,t,p] = rodbload(outfile,'yy:mm:dd:hh:c:t:p');
        if (sn(i)==3218 | sn(i)==3481 | sn(i)==8079)
            c=c*10.;
        end
...
%         ni = ~isnan(ot(i,:)); ctdot(i,ni)=interp1(d.timeJ, d.temp, jd(i,ni));
        ni = ~isnan(c(i,:)); ctdc(i,ni)=interp1(new_time, cond, jd(i,ni));
        ni = ~isnan(t(i,:)); ctdt(i,ni)=interp1(new_time, temp, jd(i,ni));
        ni = ~isnan(p(i,:)); ctdp(i,ni)=interp1(new_time, press, jd(i,ni));
    end
end
...
ylabel('conductivity')
xlabel('yearday (relative to 2018/1/0 00:00)')
title(['CAST ' cast ' Calibration Dip'])
if doctd
%     plot(d.timeJ,d.cond,'k-') % bim
    plot(new_time,cond,'k-')
...
%     plot(d.timeJ,d.temp,'color',[.3 .3 .3]); % testing for sensor drift
    plot(new_time,temp,'color',[.3 .3 .3]);
    title(['CAST ' cast ' Calibration Dip (-k=CTD)'])
end
xlim([jdstt-0.01 jdend+0.01]);
orient tall
