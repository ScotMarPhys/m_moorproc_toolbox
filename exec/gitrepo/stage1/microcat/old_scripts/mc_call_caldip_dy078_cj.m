...
cast = '2'; dateoffsetmcdefault = 2018; % origine of the julian day in the matlab cnv files
...
ctdinfile = [ctddir 'ctd_dy078_' ctdnum '_psal.nc'];
...
% apply basic oxygen calibration (extra code Clare Johnson, Nov 2018)
for i=1:length(vec)
    ratio = 1.2352e-5 * ctdp(i,:) + 1.0433;
    cctdo2(i,:) = ctdo2(i,:) .* ratio;
end
...
for j=1:length(vec);
  ...
  ylim([-10 10]);
    ...
    title(['s/n : ',num2str(vec(j)),' with 1st look ctd oxy calib applied']);
