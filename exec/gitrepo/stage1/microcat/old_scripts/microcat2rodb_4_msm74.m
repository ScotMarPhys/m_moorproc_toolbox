% function microcat2rodb_3('infile','outfile','infofile',fidlog,[graphics],[dateoffset])
%
...
%
% nph     16 June 2018 Add a line to convert units of cond as GEOMAR script
%                  output data in wrong units

function microcat2rodb_4(infile,outfile,infofile,fidlog,graphics,dateoffset);
...
%         SerialNumber = str2num(zeile(a+17:a+20));  
        SerialNumber = str2num(zeile(a+17:a+21));  % Changed DY053 to account for 5 digit s/n
    else
        disp('unable to find serial number\n\n')  
        input('please enter valid serial number')
        SerialNumber = str2num(input('please enter valid serial number'));
...
% wrong format
if (strfind(outfile,'d345') & strfind(outile,'7347_cal_dip'));
      check1=1;
else
      check1=0;
end
...
% MSM74 two older instruments produce data in S/m so need to prepare for later
% correction for different units
if check1|(strfind(outfile,'msm74') & strfind(outfile,'3218'));
    dt(:,2)=dt(:,2)*10;
    disp('s/n 3218 has cond in S/m')
end
if check1|(strfind(infile,'msm74') & strfind(infile,'3481'));
    dt(:,2)=dt(:,2)*10;
    disp('s/n 3481 has cond in S/m')
end
if check1|(strfind(infile,'msm74') & strfind(infile,'8079_cal_dip'));
    dt(:,2)=dt(:,2)*10;
    disp('s/n 8079 cal dip has cond in S/m')
end
...
if cnv==0 % data is in .asc file
  if size(dt,2)==9 % time in dd MMM yyyy format 
      HH=hms2h(dt(:,7),dt(:,8),dt(:,9));     
      jd=julian(dt(:,6),dt(:,5),dt(:,4),HH);
      gtime=gregorian(jd);
    ...
    if size(dt,2)==5 % SMP with pressure
        ...
        jd=cnv_days+julian(dateoffset,1,1,0) - 1;
        disp(['dateoffset : ',num2str(dateoffset)]);
        gtime=gregorian(jd);
        HH=hms2h(gtime(:,4),gtime(:,5),gtime(:,6));
        Start_Date = gtime(1,[1 2 3]);
        End_Date = gtime(dtl,[1 2 3]);
...
if size(dt,2) ==8
 ...
 % SCU JYM 20/4/05 Changed to CSIRO seawater routines
  ...
  % for MSM the GEOMAR script output cond in wrong format, except for old
  % instruments 3218 and 3481, so divide by 10 here
