function mc_caldip_check_pe400
...
% check if m_setup already started and if not start it now
if isempty(whos('global','MEXEC_A'))
    disp('m_setup not already run so running now.')
    m_setup
end
...
cruise = 'pe400'; % used for microcat data
cruise2='pe400'; % used for ctd data
...
%  [d h]=mload(ctdinfile,'timeJ','prDM','t090C','c0S_slash_m',' ');
 [d h]=mload(ctdinfile,'time','press','temp1','cond1',' ','q');
