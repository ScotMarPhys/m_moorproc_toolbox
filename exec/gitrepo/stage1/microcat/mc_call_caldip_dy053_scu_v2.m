...
cast = '22'; dateoffsetmc = 2016; % origine of the julian day in the matlab cnv files
...
basedir = [pathosnap,'/','data','/'];
ctddir = ['/home/mstar/dy053/data/ctd/']; 
...
firinfile = [ctddir 'fir_' cruise_ctd '_' ctdnum '_time.nc'];

%% Load CTD and bottle firing data
if doctd == 1;    
    [d h]=mload(ctdinfile,'/');
    [dfir hfir]=mload(firinfile,'/');
    ...
    dfir.timeJ=julian(jtime)+dfir.time/86400-jd0;
end
%% Load mooring data
...
%     fprintf(fidlog,'\n\n');
    ...
    if id2(i)~=335 % checks if not ODO microcat
        ...
        ixjd = jd >jdstt & jd < jdend; % start/end times from info.dat file
        c(~ixjd) = NaN; % make up a matrix of the mcat data all the same length
        t(~ixjd) = NaN;
        p(~ixjd) = NaN;
...
for i = 1:length(vec)
    ...
    if doctd
        ni = ~isnan(c(i,:)); ctdc(i,ni)=interp1(d.timeJ, d.cond1, jd(i,ni));
        ni = ~isnan(t(i,:)); ctdt(i,ni)=interp1(d.timeJ, d.temp1, jd(i,ni));
        ni = ~isnan(p(i,:)); ctdp(i,ni)=interp1(d.timeJ, d.press, jd(i,ni));
        ni = ~isnan(ot(i,:)); ctdot(i,ni)=interp1(d.timeJ, d.temp1, jd(i,ni));
    end
end
