...
YEAR=2017 ;% set to current year or year for which processing being completed
jd0 = julian(YEAR,1,0,0);


ctdnum = sprintf('%03d',str2num(cast));
if exist('/local/users/pstar/rpdmoc/rapid/data/exec/jc145/stage1/microcat/mc_call_caldip_v4.m','file')
    % on cruise jc145
    basedir = '/local/users/pstar/rpdmoc/rapid/data/';
    ctddir = '/local/users/pstar/cruise/data/ctd/';
elseif exist('/Volumes/rpdmoc/rapid/data/exec/jc145/stage1/microcat/mc_call_caldip_v4.m','file')
    % using DR Mac with mount to banba on JC145
    basedir = '/Volumes/rpdmoc/rapid/data/';
    ctddir = '/Volumes/rpdmoc/jc145/data/ctd/';
else % at noc
    basedir = '/noc/mpoc/rpdmoc/rapid/data/';
    ctddir = '/noc/mpoc/rpdmoc/jc145/data/ctd/';
end
...
if doctd == 1;
    if isempty(whos('global','MEXEC_A')) % simple check to see if m_setup already been run for this session to avoid running again
        m_setup;
    end
    [d h]=mload(ctdinfile,'/');
...
for i = 1:length(vec)
    ...
    if exist(infile)
        ...
        microcat2rodb_3(infile,outfile,infofile,fidlog,'y',0)
