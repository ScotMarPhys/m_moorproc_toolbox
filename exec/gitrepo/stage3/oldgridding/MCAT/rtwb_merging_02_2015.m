...
boundarydir = [basedir 'users/loh/moor_merging_gridding/'];
...
mc_check_plot   = true; %false ;  % turns on/off the microcat check plots. off =flase
...
% % If a merge product of RTWB is available for this time period: 
...
print('-dpng',[grdatdir 'otherfigure' filesep  outputfile '_beforegrid_check'])
...
elseif gridding == 1 % linear interpolation   
    ...
    RTWB_merg.TGfs   = TGfs;
    RTWB_merg.SGfs   = SGfs;
%     RTWB_merg.P_sort = P_sort;
%     RTWB_merg.T_sort = T_sort;
%     RTWB_merg.S_sort = S_sort;
if gridding == 2 % using climatological profiles    
...
%     RTWB_merg.OUT_FILE         = OUT_FILE;
%     RTWB_merg.EB_creation_date = datestr(now);
%     RTWB_merg.MERG_REVISION    = REV(6:8);            % store the revision number of this script
%     RTWB_merg.MERG_AUTHOR      = REV_AUTHOR(9:end-1); % store the revision author of this script
%     RTWB_merg.MERG_DATE        = REV_DATE(7:end-1);   % store the revision date of this script
%     RTWB_merg.EB_path = grdatdir; % path to output file
%     RTWB_merg.function_name = function_name; % the name and path to this function
%     RTWB_merg.interpolation_depth=idepth-20;
%     RTWB_merg.matlab_version = version;
%     
% % bim September 2014
...
% 	end
%     RTWB_merg.climatology = clim; % structure of the climatology variables used to grid the data
% 
%     RTWB_merg
...
%
    ...
    figure(1); clf
    subplot(3,1,1)
    contourf(JG , pgg, TGfs); axis ij
    datetick; ylabel('TEMPERATURE')
    title('BEFORE DESPIKING AND INTEROLATION')
    subplot(3,1,2)
    contourf(JG , pgg, temp); axis ij
    datetick; ylabel('TEMPERATURE');
    title('AFTER DESPIKING')
    subplot(3,1,3)
    contourf(JG , pgg, TG_east); axis ij
    datetick; ylabel('TEMPERATURE')
    ...
    figure(2);clf
    subplot(3,1,1)
    contourf(JG , pgg, SGfs); axis ij
    datetick; ylabel('SALINITY')
    title('BEFORE DESPIKING AND INTEROLATION')
    subplot(3,1,2)
    contourf(JG , pgg, salinity); axis ij
    datetick; ylabel('SALINITY')
    title('AFTER DESPIKING')
    subplot(3,1,3)
    contourf(JG , pgg, SG_east); axis ij
    datetick; ylabel('SALINITY')
    title('AFTER DESPIKING AND INTERPOLATION')
