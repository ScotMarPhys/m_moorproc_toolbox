...
% _*Updated by Sam Diabate for ODISSEA, 2026*
...
jg_start                = datenum(2024,01,01,00,00,00);
jg_end                  = datenum(2025,12,31,00,00,00);
...
% SAM NOTES: No info.dat file. Where am I meant to  find microcat_order?
microcat_order=({[1:9]});
...
% ODISSEA instrument_order is:
instrument_order=([9141 7923 5509 5510 16363 16379 21269 23829 23830]);
...
p_hydrogrid.ini_path     = [p_hydrogrid.basedir '/odissea/data/moor/proc/hydro_grid/ini'];
p_hydrogrid.rodbpath     = [p_hydrogrid.basedir '/odissea/data/moor/'];
p_hydrogrid.rodbctdpath  = [p_hydrogrid.basedir '/odissea/cruise_data/'];
p_hydrogrid.info_path    = [p_hydrogrid.rodbpath,'proc/',moor,'/'];
p_hydrogrid.mooringpath  = [basedir '/odissea/data/moor/proc'];
p_hydrogrid.out_path     = [p_hydrogrid.basedir '/odissea/data/moor/proc/hydro_grid/'];
p_hydrogrid.outname      = [moor,'_grid.mat'];
p_hydrogrid.dataexternal_ctd_dir        = [p_hydrogrid.basedir '/odissea/cruise_data/'];
...
case 'Merge'
    ...
    % file name and removing number after rteb
    for ii=1:numel(moor)
       moor{ii}=[moor{ii}(1:4) '_odissea' moor{ii}(6:end)];
    end
    ...
    switch answer
        ...
        case 'Wrong moorings'
            ...
            % file name
            for ii=1:numel(moor)
               moor{ii}=[moor{ii}(1:5) '_odissea' moor{ii}(6:end)];
            end
