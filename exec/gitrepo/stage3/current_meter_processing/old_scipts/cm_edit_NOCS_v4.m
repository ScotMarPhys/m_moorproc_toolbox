...
%
% Edited by Loic Houpert, Feburary 2019: - add the option to use data from nearby microcat to correct the speed of sound (just enter the serial number of the sbe37)
%					 - edit the option asking for optional PGOOD argument for ADCP data
%					 - If it exists, load a matlab file containing the good data bin domain. 
%                  This type of matlab file is generated by the script stage1/adcp/read_flag_raw_adcp.m 
...
if iiADCP>0 
    ...
    for i= 1:length(vecADCP);
       ...
       % read data into vectors and then into structure array if required
       if exist(outfilenonfilt) == 2 
         over = 'y';%input([outfile,' exists already: do you want to overwrite it? y/n '],'s');  
       ...
       if j==1 | ~exist('corfac','var') % calculation of the corrected speed of sound for the 1st bin at the transducer depth. 
           % If there are no data available for the 1st bin, calculate it
           % for next good bin
           %t68=t*1.00024; % convert T90 temp values to T68 for use with CSIRO routines
           s(1:length(t),1)=use_c; %creates vector of same length as t filled with use_c value for fixed salinity.
           svel = sw_svel(s,t,p);
