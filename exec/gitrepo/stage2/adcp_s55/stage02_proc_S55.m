...
stage2_block = sprintf([
    '%s: Stage 2 processing:\n' ...
    '- %s\n- %s\n- %s\n- %s\n- %s\n- %s'], ...
    datestr(now,'yyyy-mm-ddTHH:MM:SSZ'), ...
    hist_hard_iron, ...
    hist_mag_def, ...
    '10-member burst measurements averaged into single 30-minute ensemble profiles.',...
    depth_source, ...
    sos_method, ...
    hist_sidelobe);
...
try
    ...
    stage1_history = ncreadatt(infile, '/', 'history');
