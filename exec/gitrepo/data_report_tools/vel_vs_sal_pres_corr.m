function vel_vs_sal_pres_corr(moor,varargin)
%
...
% modified by loic Houpert 12/02/2019
% modified by loic Houpert 20/10/2020 - Added velocity pressure plot

proclvl =2;

if nargin <1
    help vel_vs_sal_pres_corr
    return
end
...
depths(:,1) = id([iiS4;iiRCM11;iiARG;iiNOR;iiADCP;iiMC]);
depths(:,2) = z([iiS4;iiRCM11;iiARG;iiNOR;iiADCP;iiMC]);
...
if length(sn)~=length(depths(:,1))
    disp('%%%%%%%%%%%%%%%%%%5')
    disp('Error subselection of instruments depths doesnt match the variable sn loaded from info.dat file')
    disp(' ')
    keyboard
end
...
set(correlation_plot, 'PaperPosition', figuresize);depths(:,1)
...
matching_depth=60; % set the depth at which instruments are close enough to each other to be considered at the same depth
matching=find(diff(depths(:,2))<=matching_depth & abs(diff(depths(:,1)))==33); %produces index of matching instrument depths, but only pointing to first instrument of the pair
a=size(matching);
i=1; j=1;
...
% Pair nortek and microcat at same level
if matching_insts(1,2)==370 % if nortek is first
    for ijk=1:length(matching_insts)/2
        iselNOR(ijk) = find(vecNOR==matching_insts(0+(ijk*2-1),1));
        iselMC(ijk) =  find(vecMC==matching_insts(1+(ijk*2-1),1));
    end
elseif matching_insts(1,2)==337 % if microcat is first
     for ijk=1:length(matching_insts)/2
        iselMC(ijk) =  find(vecMC==matching_insts(0+(ijk*2-1),1));
        iselNOR(ijk) = find(vecNOR==matching_insts(1+(ijk*2-1),1));
    end   
end
...
%% PLot u vs sal (.microcat file need to have been generated!"
% eval_string2=eval_string(iii);
eval_stringA=eval_string(iiNOR(iselNOR));
eval_stringB=eval_string(iiMC(iselMC));
for i=1:length(iselNOR)
    figure(correlation_plot);
    subplot(m,n,i)
    hold on
%     eval(['plot(' char(eval_string2(i)) '.u,' char(eval_string2(i)) '.t,''.'');'])
%     title(['Current meter at ' num2str(depths(iii(i),2)) 'm.']);
%     xlabel('u velocity component (cm/s)')
%     ylabel('temperature (deg C)')
    
    xini = eval([char(eval_stringB(i)) '.s;']);
    tini = eval([char(eval_stringB(i)) '.jd;']);
    tref = eval([char(eval_stringA(i)) '.jd;']);
    
    xnew = interp1(tini,xini,tref);
    
    eval(['plot(' char(eval_stringA(i)) '.u, xnew,''.'');'])
    title(['Current meter at ' num2str(depths(iiNOR(i),2)) 'm.']);
    xlabel('u velocity component (cm/s)')
    ylabel('Salinity')
...
% end
if filt_or_unfilt==0 % i.e. filtered data
    suptitle_string=['2-day lowpass filtered salinity vs eastward velocity for mooring ' s '.'];
else
    suptitle_string=['Unfiltered salinity vs eastward velocity for mooring ' s '.'];
end
suptitle(suptitle_string)

print('-dpng',[moor '_sal_vs_u'])
savefig([moor '_sal_vs_u'])
...
% eval_string2=eval_string(iii);
eval_stringA=eval_string(iiNOR(iselNOR));
eval_stringB=eval_string(iiMC(2));
for i=1:length(iselNOR)
    figure(correlation_plot);
    subplot(m,n,i)
    hold on
...
%     ylabel('temperature (deg C)')
    ...
    xini = eval([char(eval_stringB) '.p;']);
    tini = eval([char(eval_stringB) '.jd;']);
    tref = eval([char(eval_stringA(i)) '.jd;']);
    
    xnew = interp1(tini,xini,tref);
    
    eval(['plot(' char(eval_stringA(i)) '.v, xnew,''.'');'])
    title(['V at ' num2str(depths(iiNOR(i),2)) 'm.  vs  Pres. at ' num2str(depths(iiMC(2),2)) ]);
    xlabel('V velocity component (cm/s)')
    ylabel('Pressure from near-surface mcat')
...
% end
if filt_or_unfilt==0 % i.e. filtered data
    suptitle_string=['2-day lowpass filtered near-surface pressure vs northward velocity for mooring ' s '.'];
else
    suptitle_string=['Unfiltered near-surface pressure vs northward velocity for mooring ' s '.'];
end
suptitle(suptitle_string)

print('-dpng',[moor '_pres_vs_v'])
savefig([moor '_pres_vs_v'])
