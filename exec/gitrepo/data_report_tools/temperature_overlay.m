% function temperature_overlay(moor,'procpath','proclvl','layout','plot_interval','unfiltered')
%
% Function for plotting temperatures of a mooring overlayed on the same axes
%
% required inputs:-
...
%           e.g. temperature_overlay('wb1_1_200420','layout','landscape')
%   plot_interval: matrix of start and end dates for plot
%           e.g. temperature_overlay('wb1_1_200420','plot_interval',[2004 02 01 00; 2005 06 01 00])
...
%           e.g. temperature_overlay('wb1_1_200420','inpath','/Volumes/noc/mpoc/hydro/rpdmoc/rapid/data/moor/proc/')
...
% adapted from temperature_overlay.m
...
for iid=1:length(id_z_sn.id)
    if ~isempty(id_z_sn.dirs{iid}) && contains(id_z_sn.vars{iid},'t')
        if ~non_verbose
            disp('*************************************************************')
        ...
        if fileopen>0
            [yy,mm,dd,hh,data.t] = rodbload(infile,'yy:mm:dd:hh:t');
        ...
        if ~isempty(data)
            data.t(data.t<-999) = NaN;
            if unfilt==0
                sampling_rate = 1/median(diff(data.jd));
                ii = find(~isnan(data.t));
                data.t(ii) = auto_filt(data.t(ii),sampling_rate,1/2,'low',4);
            end
            alldata.(iname) = data;
...
for iid = 1:length(id_z_sn.id)
    ...
    plot(alldata.(iname).jd-jd1,alldata.(iname).t,'color',col);
end
...
for iid = 1:length(id_z_sn.id)
    iname = sprintf('%s_%d', id_z_sn.inst{iid}, id_z_sn.sn(iid));
    label_y_positions = nanmedian(alldata.(iname).t);
    col = 'black'; if isodd(iid); col = 'blue'; end
...
outfile = fullfile(outpath,[moor '_temperature_overlay_proclvl_' proclvlstr]);
print('-dpng',outfile)
savefig(outfile)
