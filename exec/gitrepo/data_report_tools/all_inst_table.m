...
% putting in table that can be looped through or have rows extracted
...
id_z_sn.inst = cell(length(id),1);
id_z_sn.inst(iiMC) = {'MC'};
id_z_sn.inst(iiODOMC) = {'ODOMC'};
id_z_sn.inst(iiRBR) = {'RBR'};
id_z_sn.inst(iiIDR) = {'IDR'};
id_z_sn.inst(iiS4) = {'S4'};
id_z_sn.inst(iiRCM11) = {'RCM11'};
id_z_sn.inst(iiARG) = {'ARG'};
id_z_sn.inst(iiNOR) = {'NOR'};
id_z_sn.inst(iiDVS) = {'DVS'};
id_z_sn.inst(iiBPR) = {'BPR'};
id_z_sn.inst(iiSG) = {'SG'};
id_z_sn.insts(iiSP) = {'SP'};
