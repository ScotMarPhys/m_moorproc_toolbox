% function salinity_overlay(moor,'procpath','proclvl','layout','plot_interval')
%
% Function for plotting salinity from a mooring overlayed on the same axes
%
% required inputs:-
...
%
function salinity_overlay(moor,varargin)
if nargin <1
    help salinity_overlay
    return
end
...
salinity_plot=figure('Position',pos1);
set(salinity_plot,'PaperUnits','centimeters');
set(salinity_plot, 'PaperType', 'A4');
set(salinity_plot, 'PaperOrientation',layout);
papersize = get(salinity_plot,'PaperSize');
width=17; height=26; left = (papersize(1)- width)/2; bottom = (papersize(2)- height)/2;
figuresize = [left, bottom, width, height];
set(salinity_plot, 'PaperPosition', figuresize);
...
if iiIDR>0
    ...
    for i=1:length(vecIDR);
       ...
       if fileopen>0
           ...
           eval([char(eval_string(iiiIDR(j))) '.s=s;']);
           sampling_rate = 1/median(diff(jd));
           ...
           if filtered==1
               ...
               ii = eval(['find(~isnan(' char(eval_string(iiiIDR(j))) '.s));']); 
               eval([char(eval_string(iiiIDR(j))) '.s(ii)=auto_filt(' char(eval_string(iiiIDR(j)))...
                     '.s(ii), sampling_rate, 1/2,''low'',4);']);
           end
       end
       j=j+1;
...
if iiRBR>0
    ...
    for i=1:length(vecRBR);
       ...
       if fileopen>0
           ...
           eval([char(eval_string(iiiRBR(j))) '.s=s;']);
           sampling_rate = 1/median(diff(jd));
           ...
           if filtered==1
               ...
               ii = eval(['find(~isnan(' char(eval_string(iiiRBR(j))) '.s));']); 
               eval([char(eval_string(iiiRBR(j))) '.s(ii)=auto_filt(' char(eval_string(iiiRBR(j)))...
                 '.s(ii), sampling_rate, 1/2,''low'',4);']);
...
if iiS4>0 
    ...
    for i=1:length(vecS4);
       ...
       if fileopen>0
           ...
           eval([char(eval_string(iiiS4(j))) '.s=s;']);
           sampling_rate = 1/median(diff(jd));
           ...
           if filtered==1
               ...
               ii = eval(['find(~isnan(' char(eval_string(iiiS4(j))) '.s));']); 
               eval([char(eval_string(iiiS4(j))) '.s(ii)=auto_filt(' char(eval_string(iiiS4(j)))...
                     '.s(ii), sampling_rate, 1/2,''low'',4);']);
           end
       end
       j=j+1;
...
if iiRCM11>0
    ...
    for i=1:length(vecRCM11);
       ...
       if fileopen>0
           ...
           eval([char(eval_string(iiiRCM11(j))) '.s=s;']);
           sampling_rate = 1/median(diff(jd));
           if filtered==1
               % Apply a butterworth filter to the data using auto_filt and use for
               % plots
               
               ii = eval(['find(~isnan(' char(eval_string(iiiRCM11(j))) '.s));']); 
               eval([char(eval_string(iiiRCM11(j))) '.s(ii)=auto_filt(' char(eval_string(iiiRCM11(j)))...
                 '.s(ii), sampling_rate, 1/2,''low'',4);']);
           end
       end
       j=j+1;
...
if iiSG>0
    ...
    for i=1:length(vecSG);
       ...
       eval([char(eval_string(iiiSG(j))) '.s=s;']);
       sampling_rate = 1/median(diff(jd));
       if filtered==1
           
           % Apply a butterworth filter to the data using auto_filt and use for
           % plots
           ii = eval(['find(~isnan(' char(eval_string(iiiSG(j))) '.s));']); 
           eval([char(eval_string(iiiSG(j))) '.s(ii)=auto_filt(' char(eval_string(iiiSG(j)))...
                 '.s(ii), sampling_rate, 1/2,''low'',4);']);
       end
...
for i=1:length(eval_string)
    figure(salinity_plot); hold on
    eval(['plot(' char(eval_string(i)) '.jd-jd1,' char(eval_string(i)) '.s,''' colours(i) ''');']);
end

figure(salinity_plot);
ylabel('salinity');
xlim([0 jd2-jd1]);
...
if filtered==1
    title(['Low-pass filtered salinity from mooring: ' s])
else
    title(['Unfiltered salinity from mooring: ' s])
end
...
for i=1:length(eval_string)
    label_y_positions=eval(['nanmedian(' char(eval_string{i}) '.s);']); 
    eval(['text(label_x_positions, label_y_positions,[num2str(depths(i,2)) ''m''],''FontSize'',8,''color'',''' colours(i) ''');'])
end
print('-dpng',[moor '_salinity_overlay_proclvl_' proclvlstr])
savefig([moor '_salinity_overlay_proclvl_' proclvlstr])
