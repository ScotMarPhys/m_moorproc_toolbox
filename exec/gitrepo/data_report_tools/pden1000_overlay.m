% function pden1000_overlay(moor,'procpath','proclvl','layout','plot_interval')
%
% Function for plotting potential density from a mooring overlayed on the same axes
%
% required inputs:-
...
% 
% Routine written by Darren Rayner November 2008 (cruise D334).
% adapted from conductivity_overlay.m
% 15/4/11 - added compatability for Seaguards 
% 5/10/16 - Loic, H: adapt from salinity_overlay function + added option to process lvl 3 data (.microcat and .edt files for nortek) and save plot
%

function pden1000_overlay(moor,varargin)
if nargin <1
    help pden1000_overlay
    return
end
...
depths(:,1) = id([iiMC;iiRBR;iiIDR;iiS4;iiRCM11;iiSG]);
depths(:,2) = z([iiMC;iiRBR;iiIDR;iiS4;iiRCM11;iiSG]);
...
iii=[iiiS4;iiiRCM11;iiiMC;iiiRBR;iiiIDR;iiiSG];
...
pden_plot=figure('Position',pos1);
set(pden_plot,'PaperUnits','centimeters');
set(pden_plot, 'PaperType', 'A4');
set(pden_plot, 'PaperOrientation',layout);
papersize = get(pden_plot,'PaperSize');
width=17; height=26; left = (papersize(1)- width)/2; bottom = (papersize(2)- height)/2;
figuresize = [left, bottom, width, height];
set(pden_plot, 'PaperPosition', figuresize);
...
if iiMC>0
	...
	elseif proclvl==3
       ...
       if fileopen>0
           ...
           s=sw_salt(c_ratio,t,p); % version 3.2 of the Sea water routines used T-90 rather than T-68
...
if iiIDR>0
    ...
    for i=1:length(vecIDR);
       ...
       if fileopen>0
           ...
           eval([char(eval_string(iiiIDR(j))) '.pden1000=pden1000;']);
           eval([char(eval_string(iiiIDR(j))) '.ptmp1000=ptmp1000;']);
           sampling_rate = 1/median(diff(jd));
           ...
           if filtered==1
               ...
               ii = eval(['find(~isnan(' char(eval_string(iiiIDR(j))) '.pden1000));']); 
               eval([char(eval_string(iiiIDR(j))) '.pden1000(ii)=auto_filt(' char(eval_string(iiiIDR(j)))...
                     '.pden1000(ii), sampling_rate, 1/2,''low'',4);']);
               ii = eval(['find(~isnan(' char(eval_string(iiiIDR(j))) '.ptmp1000));']); 
               eval([char(eval_string(iiiIDR(j))) '.ptmp1000(ii)=auto_filt(' char(eval_string(iiiIDR(j)))...
                     '.ptmp1000(ii), sampling_rate, 1/2,''low'',4);']);
           end
       end
       j=j+1;
...
if iiRBR>0
    ...
    for i=1:length(vecRBR);
       ...
       if fileopen>0
           ...
           eval([char(eval_string(iiiRBR(j))) '.pden1000=pden1000;']);
           sampling_rate = 1/median(diff(jd));
           ...
           if filtered==1
               ...
               ii = eval(['find(~isnan(' char(eval_string(iiiRBR(j))) '.pden1000));']); 
               eval([char(eval_string(iiiRBR(j))) '.pden1000(ii)=auto_filt(' char(eval_string(iiiRBR(j)))...
                 '.pden1000(ii), sampling_rate, 1/2,''low'',4);']);
...
if iiS4>0 
    ...
    for i=1:length(vecS4);
       ...
       if fileopen>0
           ...
           eval([char(eval_string(iiiS4(j))) '.pden1000=pden1000;']);
           sampling_rate = 1/median(diff(jd));
           ...
           if filtered==1
               ...
               ii = eval(['find(~isnan(' char(eval_string(iiiS4(j))) '.pden1000));']); 
               eval([char(eval_string(iiiS4(j))) '.pden1000(ii)=auto_filt(' char(eval_string(iiiS4(j)))...
                     '.pden1000(ii), sampling_rate, 1/2,''low'',4);']);
           end
       end
       j=j+1;
...
if iiRCM11>0
    ...
    for i=1:length(vecRCM11);
       ...
       if fileopen>0
           ...
           eval([char(eval_string(iiiRCM11(j))) '.pden1000=pden1000;']);
           sampling_rate = 1/median(diff(jd));
           if filtered==1
               % Apply a butterworth filter to the data using auto_filt and use for
               % plots
               
               ii = eval(['find(~isnan(' char(eval_string(iiiRCM11(j))) '.pden1000));']); 
               eval([char(eval_string(iiiRCM11(j))) '.pden1000(ii)=auto_filt(' char(eval_string(iiiRCM11(j)))...
                 '.pden1000(ii), sampling_rate, 1/2,''low'',4);']);
           end
       end
       j=j+1;
...
if iiSG>0
    ...
    for i=1:length(vecSG);
       ...
       eval([char(eval_string(iiiSG(j))) '.pden1000=pden1000;']);
       sampling_rate = 1/median(diff(jd));
       if filtered==1
           
           % Apply a butterworth filter to the data using auto_filt and use for
           % plots
           ii = eval(['find(~isnan(' char(eval_string(iiiSG(j))) '.pden1000));']); 
           eval([char(eval_string(iiiSG(j))) '.pden1000(ii)=auto_filt(' char(eval_string(iiiSG(j)))...
                 '.pden1000(ii), sampling_rate, 1/2,''low'',4);']);
       end
...
for i=1:length(eval_string)
    figure(pden_plot); hold on
    eval(['plot(' char(eval_string(i)) '.jd-jd1,' char(eval_string(i)) '.pden1000,''' colours(i) ''');']);
end
...
if filtered==1
    title(['Low-pass filtered pot. density from mooring: ' s])
else
    title(['Unfiltered pot. density from mooring: ' s])
end
...
for i=1:length(eval_string)
    label_y_positions=eval(['nanmedian(' char(eval_string{i}) '.pden1000);']); 
    eval(['text(label_x_positions, label_y_positions,[num2str(depths(i,2)) ''m''],''FontSize'',8,''color'',''' colours(i) ''');'])
end
print('-dpng',[moor '_pden1000_overlay_proclvl_' proclvlstr])
savefig([moor '_pden1000_overlay_proclvl_' proclvlstr])
...
for i=1:length(eval_string)
    figure(pden_plot); hold on
    eval(['plot(' char(eval_string(i)) '.jd-jd1,' char(eval_string(i)) '.ptmp1000,''' colours(i) ''');']);
end
...
s = regexprep(moor,'_','\\_');
if filtered==1
    title(['Low-pass filtered pot. temp from mooring: ' s])
else
    title(['Unfiltered pot. temp from mooring: ' s])
end
...
for i=1:length(eval_string)
    label_y_positions=eval(['nanmedian(' char(eval_string{i}) '.ptmp1000);']); 
    eval(['text(label_x_positions, label_y_positions,[num2str(depths(i,2)) ''m''],''FontSize'',8,''color'',''' colours(i) ''');'])
end
print('-dpng',[moor '_ptmp1000_overlay_proclvl_' proclvlstr])
savefig([moor '_ptmp1000_overlay_proclvl_' proclvlstr])
