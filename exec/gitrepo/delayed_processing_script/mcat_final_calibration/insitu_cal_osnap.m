...
% if strcmp(cruise,'jc064')
    elseif strcmp(cruise,'dy078') | strcmp(cruise,'dy053') | strcmp(cruise,'pe399')
    ctd_file
    if sensorselec==1    
...
%  dismiss short bottle stops and take care that depths are monotonously decreasing 

bst     = find(diff(bot_start) *3600*24 > bottlestop_tmin); % neglect short  bottlestops 
bst     = [bst' length(bot_end)];



if 0   % not used for the time being
  bst2     = find(diff(bottle.pav(bst))<-bottlestop_dpmin);
  bst2     = [1 [bst2+1]];
  bst      = bst(bst2);
end
...
interval_move  = interval_move /3600/24;  % convert seconds to days
interval_move1=interval_move;
...
for stop = 1 : nstop,    % bottle_stops loop
  ...
  figure(10+stop); clf; hold on; ooo = 200/60/60/24;
   ...
   for mc = 1 : ninst,     % instrument loop
       ...
       ii_move = find(JD(:,mc)<=(bot_end(stop)+interval_move(2)) & ...
                      JD(:,mc)>=(bot_start(stop)+interval_move(1)));
       ii_move2 = find(JD(:,mc)<=(bot_end(stop)+ooo) & ...
                      JD(:,mc)>=(bot_start(stop)-2*ooo));
...
if mmmmnn>4
ii_move=find(JD(:,5)<=(bot_end(2)+interval_move(2)) & JD(:,5)>=(bot_start(2)+interval_move(1)));
Ttemp=T(ii_move,5);
Ctemp=C(ii_move,5);
else
...
if ctd_latestart_offset ~=0
  ...
  for stop = 1 : nstop, 
     ii_move = find(ctd_time<=(bot_end(stop)+interval_move(2))...
                    & ctd_time>=(bot_start(stop)+interval_move(1)));
     ii_move2 = find(ctd_time<=bot_end(stop)+ooo...
                    & ctd_time>= bot_start(stop)-2*ooo);
     figure(10+stop)
     if cstat == 1
...
% --- compute dc, dt, dp
  ...
  for i = 1:ninst,
    dt_mcdep(i) =  interp1([MP bot_p0av(:,1)'],...
                                [dt(1,i) dt(:,i)'],mcdep(i));
    dc_mcdep(i) =  interp1([MP bot_p0av(:,1)'],...
                                [dc(1,i) dc(:,i)'],mcdep(i));
    dp_mcdep(i) =  interp1([MP bot_p0av(:,1)'],...
                                [dp(1,i) dp(:,i)'],mcdep(i));
    ...
    dt_av(i)  =  mean(interp1([MP bot_p0av(:,1)'],...
                              [dt(1,i) dt(:,i)'],average_interval));
    dc_av(i)  =  mean(interp1([MP bot_p0av(:,1)'],...
                              [dc(1,i) dc(:,i)'],average_interval));
    dp_av(i)  =  mean(interp1([MP bot_p0av(:,1)'],...
                              [dp(1,i) dp(:,i)'],average_interval));

    dt_sd(i)  =  std(interp1([MP bot_p0av(:,1)'],...
                              [dt(1,i) dt(:,i)'],average_interval));
    dc_sd(i)  =  std(interp1([MP bot_p0av(:,1)'],...
                              [dc(1,i) dc(:,i)'],average_interval)); 
                          
    if ~isempty(find(i==pproblem)) % if MC press. bad, use ctd press. to correct conduct.
       dc_av_pproblem(i) = mean(interp1([MP bot_p0av(:,1)'],...
                    [dc_pproblem(1,pp_count) dc_pproblem(:,pp_count)'],average_interval));
       pp_count = pp_count + 1; 
    end    
...
if ~isempty(pproblem) 
  ...
  for i = 1 : length(pproblem),
    ...
    xlim(p_interval)
    num_legend(instr(pproblem)',sub,4)
    grid on
