...
rootdir = MOORPROC_G.moordatadir;
mcruise = MOORPROC_G.cruise;

%addpath mfiles/misc?

% Now look for positions in the data directory
pos_direct = fullfile(rootdir,'raw','moor_positions','tables');
moor_targets = fullfile(pos_direct,'moor_pos_target.dat');
fprintf(1,'loading target positions from %s',moor_targets)
%moor_inwater = fullfile(pos_direct,'moor_pos_pre_cruise.dat');
...
switch MEXEC_G.Mshipdatasystem
    ...
    case 'rvdas'
        nav.stream = 'posmv_gpgga';
        nav.vars = 'time latitude longitude altitude';
        % nav streams separated in rvdas so need to load 2 streams and
        % merger later
        nav2.stream = 'cnav_gnvtg';
        nav2.vars = 'time courseOverGround speedKnots';
        dep.stream = 'ea640_sddbs';
        dep.vars = 'time waterdepthmetre';
        gyr.stream = 'posmv_gphdt';
        gyr.vars = 'time headingTrue';
        log_S.stream = 'adcp_vdvbw';
        log_S.vars = 'time longitudinalWaterSpeed transverseWaterSpeed';
        t_origin = 0;
    case 'scs'
        ms_raw_to_sed(57);
        update_allmat;
...
if isempty(imoor)
    if strmatch(this_moor, 'ship') == 1
        ...
        if isfield(xtemp,'lat')
            mlat = xtemp.lat;
            mlon = xtemp.long;
        else
            mlat = xtemp.latitude;
            mlon = xtemp.longitude;
        end
        mdepth = NaN;
...
for icwx = 1:nicwx
    switch MEXEC_G.Mshipdatasystem
        case 'techsas'
            [ddd,~] = mtlast(nav.stream);
        case 'rvdas'
            [ddd,~,~] = mrlast(nav.stream);
            ddd.time=ddd.dnum;
        case 'scs'
    ...
    if strmatch(this_moor, 'ship') == 1
    	  switch MEXEC_G.Mshipdatasystem
        	  ...
        	  case 'rvdas'
            	  xtemp = mrlast(nav.stream);
                  xtemp.lat = xtemp.latitude;
                  xtemp.long = xtemp.longitude;
        	  case 'scs'
            	  xtemp = mslast(nav.stream);
            	  xtemp.lat  = xtemp.GPS_Furuno_GGA_lat;
            	  xtemp.long = xtemp.GPS_Furuno_GGA_lon;
        ...
        f = fullfile(work_dir,[this_moor '_map.fig']);
        if exist(f,'file') && icwx>1
            open(fullfile(work_dir, [this_moor '_map.fig']));
        else
            rapid_web_map(web_dis,work_dir,this_moor,mlat,mlon,mdepth,north,south,east,west,cont_int);
        end
    end
    ...
    switch MEXEC_G.Mshipdatasystem
        ...
        case 'rvdas'
            ...
            xdata.gndspeed=interp1(xdata2.dnum,xdata2.speedknots,xdata.dnum);
            xdata.gndcourse=interp1(xdata2.dnum,xdata2.course,xdata.dnum);
        case 'scs'
    ...
    switch MEXEC_G.Mshipdatasystem
        ...
        case 'rvdas'
            ...
            gdata.heading = gdata.headingtrue;
        case 'scs'
    ...
    sog = mean(xdata.gndspeed(end-navsec:end),'omitnan');
    ...
    switch MEXEC_G.Mshipdatasystem
        ...
        case 'rvdas'
            ...
            ycdata.forewaterspeed=ycdata.speed_forward;
            ycdata.portwaterspeed=-ycdata.speed_stbd; % I think this need the opposite sign
        case 'scs'
    ...
    wsco_f =  mean(ycdata.forewaterspeed,'omitnan');
    wsco_p =  mean(ycdata.portwaterspeed,'omitnan');
    ...
    if ~no_log_C
        ...
        wsfa =  mean(ydata.speedfa,'omitnan');
        wsps =  mean(ydata.speedps,'omitnan');
    else
    	  wsfa = wsco_f;
          wsps = wsco_p;
    end
    ...
    if web_dis == 1
        pfile = fullfile(work_dir,'latest_plot.png');
        print('-dpng',pfile);
        close(gcf);
        %   Finally move plots to the web directory
        copyfile(pfile,web_dir)
