...
% Some preliminaries
twait = 180;    % Recalculated later - time between plots
...
switch MEXEC_G.Mshipdatasystem
	...
	case 'scs'
		quickdata = quick_scs;
		quickdata.time = quickdata.dnum;
		t_origin = 0;
	case 'scsx'
		day_no = datenum(floor(now))-datenum(2023,1,1)+1;
		ms_raw_to_sed(day_no);
		update_allmat;
		t_origin = 0;
        nav.stream = 'posfur';
...
case 'scs'
	xtemp.time = quickdata.dnum;
	xtemp.lat = quickdata.lat;
	xtemp.long = quickdata.lon;
	iok = find(~isnan(xtemp.long) & ~isnan(xtemp.lat));
	xtemp.lat=xtemp.lat(iok(end));
	xtemp.long=xtemp.long(iok(end));
end
    mlat = xtemp.lat;
	mlon = xtemp.long;
    mdepth = NaN;
...
% Neeed to get some navigation data etc.  Get GPS first
  ...
  switch MEXEC_G.Mshipdatasystem
    ...
    case 'scs'
        xdata.time = quickdata.time;
		ixx = xdata.time > t0-0.0417 & xdata.time < t0;
		if sum(ixx) < 100;
			disp('No data')
			keyboard
		end
		xdata.time = quickdata.time(ixx);
        xdata.lat  = quickdata.lat(ixx);
        xdata.long = quickdata.lon(ixx);
        xdata.gndspeed = NaN*xdata.lat;
        xdata.gndcourse= NaN*xdata.lat;
  end
...
% depth data
  switch MEXEC_G.Mshipdatasystem
    ...
    case 'scs'
        zdata.time = quickdata.dnum(ixx);
        zdata.depthm = quickdata.watdep(ixx);
  end
...
% Gyro data
  switch MEXEC_G.Mshipdatasystem
    ...
    case 'scs'
        gdata.time = quickdata.time;
        gdata.heading = quickdata.gyr;
  end
  ...
  switch MEXEC_G.Mshipdatasystem
    ...
    case 'scs'
        ycdata.time = quickdata.time;
        ycdata.forewaterspeed = quickdata.speedfa;
        ycdata.portwaterspeed = quickdata.speedps; 
  end
