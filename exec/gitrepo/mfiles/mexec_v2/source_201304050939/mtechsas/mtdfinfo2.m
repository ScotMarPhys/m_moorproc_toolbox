function  mess = mtdfinfo(instream,tonly)
...
if strncmp(tonly,'f',1)
    % fast option
    if ~isnan(num)
        mess = sprintf('   %8s   %03d %8s  %s  %03d %8s   %8s  %s\n',str1,daynum1,str1a,'to',daynum2,str2a,str2,tstream);
    else
        mess = sprintf('   %23s  %2s  %23s  %s\n',' ',' ',' ',tstream);
    end
else
    if num > 0
        mess = sprintf('%10d   %8s   %03d %8s  %s  %03d %8s   %8s  %s\n',num,str1,daynum1,str1a,'to',daynum2,str2a,str2,tstream);
    else
        mess = sprintf('%10s   %23s  %2s  %23s  %s\n','No data   ',' ',' ',' ',tstream);
    end
end

% fprintf(MEXEC_A.Mfidterm,'%s',mess);