...
%bak fixes before dy031 15 may 2015

if isempty(t1) 
    % no data
    pdata = [];
    punits = [];
    fprintf(2,'%s\n',['no data found on stream : "' instream '"']);
    return
end
...
[pdata punits] = mtload(tstream,dn,dn,'/','q');
% matlabdate = datevec(pdata.time+MEXEC_G.Mtechsas_torg);

% dvnow = datevec(now);
dvnow = datevec(dn); % bak preparing for dy031 14 may 2015. Sets origin to be year of latest data
yyyy = dvnow(1);
% yyyy = MEXEC_G.MDEFAULT_DATA_TIME_ORIGIN(1); % bak preparing for dy031 14 may 2015. Sets origin to be origin from m_setup
