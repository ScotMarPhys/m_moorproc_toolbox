...
% fix by bak and sfg before dy031 27 May 2015; attempt to get mday_00 to
% exit gracefully if no techsas netcdf files for this stream
if isempty(vars) % no no vars returned by mtgetvars; probably no techsas files exist for this stream
    return
end
...
% bak at nocs preparing for dy031 15 May 2015 ; this fix previously present
% in scs processing
...
%     techsas_units = nc_attget(techsas_in.name,techsas_names{k},'units');
    ...
    if strcmp(techsas_names{k},'time')
        techsas_data = 86400*(techsas_data + MEXEC_G.Mtechsas_torg - datenum(MEXEC_G.MDEFAULT_DATA_TIME_ORIGIN));
