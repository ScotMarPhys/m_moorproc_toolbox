function mtgaps(instream,g,dn1,dn2,mode)
% function mtgaps(instream,g,dn1,dn2,mode)
%
% USE, eg
...
for kf = 1:nf
    fn = fnames{kf};
    fullfn = [MEXEC_G.Mtechsas_root '/' fn];
...
ttime = vuse;
mtime = MEXEC_G.Mtechsas_torg + ttime;
mtime = [dn1 mtime(:)' dn2];
dtime = diff(mtime)*86400; % time difference in seconds
kgaps = find(dtime > g);
ng = length(kgaps);
if ng > 0
    dvnow = datevec(mtime(kgaps(1))); % bak preparing for dy031 14 may 2015. Sets origin to be year of earliest data
else
    %     dvnow = datevec(now);
    dvnow = MEXEC_G.MDEFAULT_DATA_TIME_ORIGIN; % bak preparing for dy031 14 may 2015. Sets origin to be origin from m_setup
end
yyyy = dvnow(1);
