...
%
% JC032, new options to enable multiple plots per page, each with
%  a different colormap. Fields: ctabstart, ctabend, ctabskip.
% ctabstart and ctabend are indices in the colortable.
%
% If cdfin.colortable IS NOT a field, create a new colortable using jet,
% and add it to any existing colormap; index the new colortable by setting
% ctabstart and ctabend.
% 
% If cdfin.colortable IS a field:
%   If ctabskip == 1, extend/trim the new colortable to the correct length.
%                     append the new colortable after the old colormap;
%                     index the new colortable by setting.
%   If ctabskip == 0, and ctabstart/end exist, then use colors from the
%                    indexed part of cdfin.colortable. Expand/trim to fit the clevs, and
%                    reset ctabstart/end.
...
colorbar_limits = [0 1]; % jc032 keep for compatibility with later code
color_level_boundaries = clev;



if isfield(cdfin,'ctabskip')
    ctabskip = cdfin.ctabskip;
else
    ctabskip = 0;
end

% JC032, new options to enable multiple plots per page, each with
%  a different colormap. ctabstart, ctabend, ctabskip.
% ctabstart and ctabend are indices in the colortable.
%
% If cdfin.colortable IS NOT a field, create a new colortable using jet,
% and add it to any existing colormap; index the new colortable by setting
...
if isfield(cdfin,'colortable')
    ...
    if (ctabskip == 1) % assume all new colors
        colortabold = colormap;
        nc = length(clev)-1;
        
        colortablepart = colortable;
        while size(colortablepart,1) < length(clev)-1 % jc032
            m = ['Colortable was too short' sprintf(' %d',size(colortablepart,1)) ' provided;' sprintf(' %d',length(clev)-1) ' required;'] ;
            m1 = '   Cycling round from start; some colors will be repeated';
            fprintf(MEXEC_A.Mfider,'%s\n',m,m1);
            colortablepart = [colortablepart; colortable]; % jc032
        end
        colortablepart = colortablepart(1:length(clev)-1,:);
        colortableuse = colortablepart; % new colortable
        colortable = [colortabold; colortableuse];
        ctabstart = size(colortable,1)-size(colortableuse,1)+1;
        ctabend = size(colortable,1);
        ctabskip = 0; % so this new section of colortable will be used next time
    elseif isfield(cdfin,'ctabstart') & isfield(cdfin,'ctabend')
        ctabstart = cdfin.ctabstart;
        ctabend = cdfin.ctabend;
        ctabskip = 0;
        % master colortable unchanged
        xc = [ctabstart ctabend]; ctabstart = min(xc); ctabend = max(xc); % ensure end > start
        if ctabend > size(colortable,1)
            m1 = ['You are tyring to use a ctabend index ' sprintf('%d',ctabend)];
            m2 = ['greater than the size of the input colortable ' sprintf('%d',size(colortable,1))];
            fprintf(MEXEC_A.Mfider,'%s\n',' ',m1,m2,' ');
            error('exiting mcontr')
        end
        colortablepart = colortable(ctabstart:ctabend,:);
        while size(colortablepart,1) < length(clev)-1 % jc032
            m = ['Colortable was too short' sprintf(' %d',size(colortablepart,1)) ' provided;' sprintf(' %d',length(clev)-1) ' required;'] ;
            m1 = '   Cycling round from start; some colors will be repeated';
            fprintf(MEXEC_A.Mfider,'%s\n',m,m1);
            colortablepart = [colortablepart; colortable(ctabstart:ctabend,:)]; % jc032
        end
        colortablepart = colortablepart(1:length(clev)-1,:);
        colortableuse = colortablepart;
        cp1 = colortable(1:ctabstart-1,:);
        cp2 = colortableuse;
        cp3 = colortable(ctabend+1:end,:);
        colortable = [cp1;cp2;cp3];
        ctabend = ctabstart+size(colortableuse,1)-1; % expand the color table into the required slot. This could have consequqnces for other cdfs with pre-defined start and stop points
    else % no ctabstart or ctabend
        % colortable exists but start and end index do not. Expand
        % colortable for this set of clevs
        colortablepart = colortable;
        while size(colortablepart,1) < length(clev)-1 % jc032
            m = ['Colortable was too short' sprintf(' %d',size(colortablepart,1)) ' provided;' sprintf(' %d',length(clev)-1) ' required;'] ;
            m1 = '   Cycling round from start; some colors will be repeated';
            fprintf(MEXEC_A.Mfider,'%s\n',m,m1);
            % fix by bak on jr281 march 2013. if we're in this branch,
            % ctabstart and ctabstop are no defined
            % colortablepart = [colortablepart; colortable(ctabstart:ctabend,:)]; % jc032
            colortablepart = [colortablepart; colortable]; % jr281
        end
        colortablepart = colortablepart(1:length(clev)-1,:);
        colortableuse = colortablepart;
        colortable = colortableuse;
        ctabstart = 1;
        ctabend = size(colortable,1);
        ctabskip = 0;
    end
else
    % no colortable provided; create one of the correct length for these contours
    colortabold = colormap;
    nc = length(clev)-1;
    colortableuse = jet(nc);
    colortable = [colortabold; colortableuse];
    ctabstart = size(colortable,1)-size(colortableuse,1)+1;
    ctabend = size(colortable,1);
    ctabskip = 0;
%     colortable = colortableuse; % save this color table
end
cdfot.colortable = colortable; %
cdfot.ctabstart = ctabstart;
cdfot.ctabend = ctabend;
cdfot.ctabskip = ctabskip;
...
for k = 1:length(yt);
    ...
    if isfield(cdfin,'ytickset')
        if strcmp(cdfin.ytickset,'none'); break; end
    end
    ht = text(0,yt(k),str);
    ...
    if isfield(cdfin,'ytickset')
        if ~isempty(cdfin.ytickset)
            cmd = ['set(ht,' cdfin.ytickset ');']; eval(cmd);
        end
    end   
end
...
for k = 1:length(xt);
    ...
    while strcmp(str(end),'.') == 1; str(end) = []; end
    while strcmp(str(1),' ') == 1; str(1) = []; end
    if isfield(cdfin,'xtickset')
        if strcmp(cdfin.xtickset,'none'); break; end
    end
    ht = text(xt(k),0,str);
    set(ht,'units','centimeters')
    pp = get(ht,'position');
    ...
    if isfield(cdfin,'xtickset')
        if ~isempty(cdfin.xtickset)
            cmd = ['set(ht,' cdfin.xtickset ');']; eval(cmd);
        end
    end   
end
...
while 1
    if isfield(cdfin,'titleset')
        if strcmp(cdfin.titleset,'none'); break; end
    end
    ...
    if isfield(cdfin,'titleset')
        if ~isempty(cdfin.titleset)
            cmd = ['set(hh,' cdfin.titleset ');']; eval(cmd);
        end
    end
    break
end
...
while 1
    if isfield(cdfin,'filenameset')
        if strcmp(cdfin.filenameset,'none'); break; end
    end
    ...
    set(hh,'VerticalAlignment','bottom')
    if isfield(cdfin,'filenameset')
        if ~isempty(cdfin.filenameset)
            cmd = ['set(hh,' cdfin.filenameset ');']; eval(cmd);
        end
    end
    break
end
...
if strncmp(newfigure,'n',1)
    ...
    while 1
        if isfield(cdfin,'dateset')
            if strcmp(cdfin.dateset,'none'); break; end
        end
...
while 1
    if isfield(cdfin,'xlabelset')
        if strcmp(cdfin.xlabelset,'none'); break; end
    end
...
while 1
    if isfield(cdfin,'ylabelset')
        if strcmp(cdfin.ylabelset,'none'); break; end
    end
    hh = text(0,0,str);
    set(hh,'units','centimeters')
    ...
    set(hh,'interpreter','none');
    if isfield(cdfin,'ylabelset')
        if ~isempty(cdfin.ylabelset)
            cmd = ['set(hh,' cdfin.ylabelset ');']; eval(cmd);
        end
    end
    break
end
...
% % % % % % % 
% % % % % % % %The size of the block is chosen with a Highest Factor routine.
