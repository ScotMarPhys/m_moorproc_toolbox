...
% % %     m1 = sprintf('\n%s\n','Type index of any vars in this list for which time should be converted to yymodd hhmmss');
% % %     m2 = sprintf('%s\n','Return for none');
% % %     reply = m_getinput([m1 m2],'s');
% % %     cmd = ['k_tconvert = [' reply '];']; %convert char response to number
% % %     eval(cmd);
...
%         h1 = [h1 longstar(1:lenfn+1)];
    ...
    for k = 1:length(vlist)
        vnum = vlist(k);
        vnam = h.fldnam{vnum};
        % % % % %         vdata = nc_varget(ncfile.name,vnam);
...
%         if ~isempty(find(k_tconvert == k)) % then this var must be converted assuming it is decimal days
        if ktime(k) == 1 & ktconvert == 1 % then this var must be converted using its units
            % % % % %             [yy mo dd hh mm ss dayofyear] = m_time_to_ymdhms(vdata,h);
            % % % % %             vdata = [yy-2000 mo dd hh mm floor(ss+.1)];
