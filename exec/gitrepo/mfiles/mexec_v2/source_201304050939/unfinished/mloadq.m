...
%
% bak on jr302, 5 july 2014: mloadq: version of mload that doesn't echo
% things ot the screen. Maybe this could be written so that mload simply
% has a 'quiet' argument, to ensure both versions remian identical, but
% mload is changed so rarely that we can probably safely maintain two
% versions.
...
d = struct(); % GJS on DY031: Set d as empty struct in case user wants to load in no data (e.g. just wants header)
...
% m_print_header(h); % not in quiet version
