function [lat lon] = msposinfo(dn1,navstream)
...
if ~exist('navstream','var'); navstream = MEXEC_G.Mscs_default_navstream; end  % use default nav stream name
...
if ~exist('dn1','var')
    ...
    dn = pdata.time+MEXEC_G.Mscs_torg;
elseif isempty(dn1); 
    [pdata u] = mslast(tstream);
    cmd = ['lat = pdata.' navunder '_lat;']; eval(cmd);
    cmd = ['lon = pdata.' navunder '_lon;']; eval(cmd);
    ...
    pdata = msload(tstream,dn-5/1440,dn+5/1440,['time ' navminus '-lat ' navminus '-lon'],'q');

    tin = pdata.time+MEXEC_G.Mscs_torg;
